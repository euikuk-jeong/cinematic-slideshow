import { sortPhotos, type PhotoMetadata } from '../photos/photoSort';
 ...
 * 있으면 그 사진들만 재생 대상으로 삼는다(CLAUDE.md 재생 화면 규칙). 기준 순서는 촬영시간
 * 오름차순 — 순차 모드는 그대로, 랜덤 모드는 이 순서를 섞는다.
 */
export function buildPlaybackSequence(
  ...
  const base = sortPhotos(photos, 'creation_time', 'asc');
